...
class CL_DCNN(nn.Module):
    def __init__(self):
        ...
        self.features = nn.Sequential(
            *(list(list(models.vgg16_bn(True).children())[0].children())[0:33])
        )
        self.dilate_module = DilateModule()
        # self.decoder = Decoder()
        self.output = nn.Conv2d(512, 1, 1, 1, 0)
        initialize_weights(self.dilate_module.modules())
        initialize_weights(self.output.modules())
